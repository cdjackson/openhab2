...
import org.openhab.binding.zwave2.internal.protocol.AssociationGroup;
...
public class ZWaveAssociationCommandClass extends ZWaveCommandClass implements ZWaveCommandClassInitialization {
    ...
    protected void processAssociationReport(SerialMessage serialMessage, int offset) {
        ...
        if (maxAssociations == 0) {
            ...
            if (updateAssociationsNode == group) {
                ...
                this.getController().notifyEventListeners(
                        new ZWaveNetworkEvent(ZWaveNetworkEvent.Type.AssociationUpdate, this.getNode().getNodeId(),
    ...
    public SerialMessage getAssociationMessage(int group) {
        logger.debug("NODE {}: Creating new message for application command ASSOCIATIONCMD_GET group {}", this
    ...
    public SerialMessage getGroupingsMessage() {
        logger.debug("NODE {}: Creating new message for application command ASSOCIATIONCMD_GROUPINGSGET", this
                .getNode().getNodeId());
        SerialMessage result = new SerialMessage(this.getNode().getNodeId(), SerialMessageClass.SendData,
                SerialMessageType.Request, SerialMessageClass.ApplicationCommandHandler, SerialMessagePriority.Config);
    ...
    /**
     * Returns a list of nodes that are currently members of the association group. This method only returns the list
     * that is currently in the class - it does not interact with the device.
     * 
    ...
    public class ZWaveAssociationEvent extends ZWaveEvent {
        ...
        /**
         * Constructor. Creates a new instance of the ZWaveAssociationEvent class.
         * 
